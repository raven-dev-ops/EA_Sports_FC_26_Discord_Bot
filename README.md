...
1. Set up Discord channels and roles:
   ...
   - On join/startup, the bot auto-creates the `--OFFSIDE DASHBOARD--` / `--OFFSIDE REPORTS--` layout, required channels (including club managers + Pro coaches), and the Offside roles (`Coach`, `Coach+`, `Club Manager`, `Club Manager+`, `League Staff`, `League Owner`, `Free Agent`, `Pro Player`).
...
- Pro coaches report channel auto-updates with roster name, openings, and practice times.
...
- **Club Managers Portal** (`channel_manager_portal_id`): coach roles (Coach/Coach+/Club Manager/Club Manager+), roster unlock/delete, pro coaches refresh, and cap sync tools.
...
- Club Managers portal: intro embed + control panel. Buttons for Set Coach Role, Unlock Roster, Refresh Pro coaches, Toggle Pro Pin, Force Rebuild Pro, Sync Caps (Active Cycle), and Delete Roster (admin-only).
