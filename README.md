...
1. Set up Discord roles:
   - Invite the bot with permissions to manage roles, send messages, read message history, and use slash commands.
   - Manage Channels is only needed to remove legacy Offside categories.
   - On join/startup, the bot auto-creates Offside roles (`Coach`, `Coach+`, `Club Manager`, `Club Manager+`, `League Staff`, `League Owner`, `Free Agent`, `Pro Player`) and removes legacy Offside Discord channels (dashboards/listings now live on the web app).
   - Assign Coach / Club Manager roles to your coaches (Coach+ / Club Manager+ expand roster caps).
   - Checklist: `docs/public/server-setup-checklist.md`
2. Configure the bot:
   - Copy `.env.example` to `.env` and fill in the required IDs and tokens.
   - Keep `TEST_MODE=true` while validating in a staging guild (no Discord channel routing).
3. Deploy and verify:
   ...
   - Confirm roles are synced and web dashboards load for your server.
   - Submit a test roster, approve/reject, and confirm the roster status updates in the web dashboard.
...
- Web dashboards power roster/recruiting flows, staff review, and listings (Discord portals are retired).
...
- Role env overrides (optional; primary source is per-guild config created by auto-setup):
  - `ROLE_COACH_ID`
  - `ROLE_COACH_PLUS_ID` (Pro)
  - `ROLE_CLUB_MANAGER_ID` (Pro)
  - `ROLE_CLUB_MANAGER_PLUS_ID` (Pro)
  - `ROLE_LEAGUE_STAFF_ID`
  - `ROLE_LEAGUE_OWNER_ID`
  - `ROLE_FREE_AGENT_ID`
  - `ROLE_PRO_PLAYER_ID`
  - Legacy fallbacks (migration only): `ROLE_TEAM_COACH_ID`, `ROLE_SUPER_LEAGUE_COACH_ID`, `ROLE_COACH_PREMIUM_ID`, `ROLE_COACH_PREMIUM_PLUS_ID`, `ROLE_RECRUIT_ID`, `ROLE_FREE_PLAYER_ID`, `ROLE_PREMIUM_PLAYER_ID`
- Channel env overrides (legacy; Discord portals/listings are retired):
  ...
  - Legacy roster listing envs: `CHANNEL_ROSTER_LISTING_ID`, `CHANNEL_ROSTER_PORTAL_ID`
...
When `TEST_MODE=true`, only logging and safety checks run (no Discord channel routing).
...
- Staff review occurs in the web dashboard; approved rosters are recorded in the database for web visibility.
