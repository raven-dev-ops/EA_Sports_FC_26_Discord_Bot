...
1) **Roster lifecycle**
   ...
   - Verify approved roster posts to roster portal; staff card cleans up on decision.
2) **Tournament flow**
...
For automation, stub Discord interactions with the integration tests pattern in `tests/integration/`, or wire a test bot token in a staging guild and run the above sequentially. Keep TEST_MODE on and use the test channel for noise.
